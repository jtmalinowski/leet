  ...
  int shortestPathAllKeys(vector<string>& grid) {
    ...
    for (int y = 0; y < maxY; y++) {
      for (int x = 0; x < maxX; x++) {
        if (grid[y][x] == '@') { startX = x; startY = y; }
        if (grid[y][x] >= 'A' && grid[y][x] <= 'F') { allKeys++; }
      }
    }
    ...
    while (!paths.empty()) {
      Path path = paths.front(); paths.pop();
      int y=path.pos.first; int x=path.pos.second;
      ...
      for (auto d : dirs) {
        ...
        int cmask = path.mask;
        if (black.count({cy,cx,cmask}) > 0) { continue; }
        ...
        if (grid[cy][cx] >= 'a' && grid[cy][cx] <= 'f') {
          ...
          if ((cmask & allKeysMask) == allKeysMask) { return csteps; }
        }

        if (grid[cy][cx] >= 'A' && grid[cy][cx] <= 'F') {
          if ( ( (1 << (grid[cy][cx] - 'A') ) & cmask ) == 0) { continue; }
        }
